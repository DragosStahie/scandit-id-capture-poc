...
class ScanFragment private constructor() : Fragment() {
    ...
    private fun setUpIdCapture() {
        ...
        acceptedDocuments.add(IdCard(IdCaptureRegion.GERMANY))

        // Regional documents:
//        acceptedDocuments.add(ApecBusinessTravelCard()) - not found

        // Reject passports from certain regions:
        rejectedDocuments.add(Passport(IdCaptureRegion.CUBA))
...
//        settings.scannerType = SingleSideScanner(true.also { barcode = it })

        // or
        settings.scannerType = SingleSideScanner(true.also { machineReadableZone = it })

        // or
//        settings.scannerType = SingleSideScanner(true.also { visualInspectionZone = it })


        // To scan both sides of the document:
//        settings.scannerType = FullDocumentScanner()

        binding.captureView.dataCaptureContext = captureContext
        val overlay = IdCaptureOverlay.newInstance(idCapture, dataCaptureView)
//        DataCaptureView.newInstance(requireContext(), captureContext)
    }
